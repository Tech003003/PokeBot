...
## What's been implemented (2026-04-24 patch)
- **Multi-browser / multi-account support**: new `browsers` table + CRUD API at `/api/browsers`. Default row auto-seeded on `init_db`. MonitorEngine now keeps `self.sessions: {browser_id → {browser, context, cdp_url}}`. `connect_brave`/`disconnect_brave`/`_new_page` all accept `browser_id`. Watchlist and Drops gained a `browser_id` column (nullable → falls back to default) and matching dropdown in the React modals. Server-side guards forbid deleting the default row or unsetting the only default.
- **Launch helper**: `/api/browsers/{id}/launch` spawns a detached Brave with `--remote-debugging-port=<port>`, `--user-data-dir=<path>`, and optional `--proxy-server=<proxy>`. Frontend: new **Browsers** tab (with big `+ Add Proxy Browser` button) showing name / CDP URL / profile dir / proxy / workers / status, plus 🚀 Launch / 🔌 Connect / ✏ Edit / 🗑 Delete. Windows helper `local/Start-Brave-Profile.bat <port> <profileDir> [proxy]` added for manual launches.
- **Stop-button crash fix**: `stop_item` / `stop_all` / `cancel_drop` now catch `asyncio.CancelledError` (it's a `BaseException`, not `Exception`, so it was escaping the existing `try/except Exception`). Regression test `test_engine_stop.py`.
- **Poll defaults bumped**: `poll_interval_ms` 700 → 50, `reload_every_n_polls` 10 → 20. Migration auto-promotes existing DBs that still hold the old defaults; user overrides preserved.
- **Tab-close-on-success fix**: `_monitor_item` now only auto-closes its Playwright page on genuine cleanup (crash/cancel/OOS). `keep_page_open=True` is set on every success-exit so the user's Brave tab stays visible to finish checkout / verify the hit. Same treatment in drop `hammer`. Regression test `test_tab_persistence.py`.
- **Tighter positive ATC verification**: removed loose `div:has-text('Added to Cart')` / `:visible` fallbacks from `_ATC_SUCCESS_SELECTORS` on Amazon / TCGPlayer. `verify_atc_success` no longer trusts a post-click cart-count number when pre-count was unreadable — it now requires an explicit success modal in that case. New regression test `test_preorder_with_error_banner_does_not_false_confirm`.
- **README**: added "What's new" summary, new `Browsers` tab row in the dashboard-tour table, new **Section 11 — Multi-account / proxy browsers** with full walkthrough, new `Browser` columns on the Watch and Drop forms, updated Engine defaults table (50 ms poll / 20-poll reload / new ATC retry cap row).
- **Tests**: 55/55 green (39 prior + 16 new for browsers API + tab-persistence + stop-cancel regressions).
